...
<?php foreach ($tmpArr as $dataarr):?>
    <?php foreach ($dataarr as $v):?>
        <tr>
            ...
            <td><?=$v['shopprice']?>元</td>
            <td>
                <input class="btn_reduce" data-link="<?=\yii\helpers\Url::to(['goods/oprater','type'=>'reduce'])?>" data-id="<?=$v['id']?>" value="-" type="button"><input value="<?=$v['gnum']?>" style="width: 20px;text-align: center" readonly><input class="btn_add" data-link="<?=\yii\helpers\Url::to(['goods/oprater','type'=>'add'])?>" data-id="<?=$v['id']?>" value="+" type="button">
            </td>
